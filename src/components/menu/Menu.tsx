...
export function Menu() {
  ...
  // Content
  const menuComponents = menuItems.map((item) => <MenuItemComponent {...item} key={item.label} />);
  const footerMenuComponents = footerMenuItems.map((item) => <MenuItemComponent {...item} key={item.label} />);
